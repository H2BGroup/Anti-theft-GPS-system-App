...
Future<void> interactiveCallback(Uri? uri) async {
  if (uri?.host == 'arm') {
    deviceArmed = true;
    await saveDeviceArmedStatus(deviceArmed);
    sendMessage(deviceArmed);
  } else if (uri?.host == 'disarm') {
    deviceArmed = false;
    await saveDeviceArmedStatus(deviceArmed);
    sendMessage(deviceArmed);
  }
...
Future<void> saveDeviceArmedStatus(bool armed) async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  await prefs.setBool("deviceArmedKey", armed);
  await HomeWidget.updateWidget(name: "RemoteWidget");
}
...
class DevicePageState extends State<DevicePage>
  ...
  static ValueNotifier<bool> stopRefreshingAnimationNotifier =
    ...
    if (AppViewState.selectedIndex == 1) {
      ...
      if (battery != null && charging != null && mounted) {
        setState(() {
          ...
          if (stringDate != null) {
            statusUpdateDate = DateFormat("yyyy-MM-dd HH:mm:ss")
  ...
  IconData returnBatteryStatusIcon() {
    if (batteryPercentage != null) {
      ...
      if (batteryPercentage! <= 32) {
        return Icons.battery_2_bar;
      }
      if (batteryPercentage! <= 48) {
  ...
  void refreshDeviceStatus() {
    ...
    if (AppViewState.selectedIndex == 1 && mounted) {
      ...
      stopRefreshingAnimationNotifier.addListener(() {
        if (stopRefreshingAnimationNotifier.value && mounted) {
          refreshButtonController.stop();
        }
      });
    }
  }
          ...
          child: Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  border: Border.all(color: darkestBlue, width: 5)),
              child:
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(25),
                      border: Border.all(color: darkestBlue, width: 3)),
                  child: Column(
                    children: [
                      Text(
                        (deviceArmed) ? "Device: Armed" : "Device: Disarmed",
                        style: TextStyle(
                            foreground: Paint()..color = Colors.white,
                            fontSize: 24,
                        ...
                        child: FittedBox(
                          ...
                          child: Row(children: [
                            Switch(
                                ...
                                trackOutlineColor: deviceArmed
                                    ? WidgetStateProperty.all(darkestBlue)
                  ...
                  child: Column(children: [
                    Text(
                      (batteryPercentage != null)
                          ? "Battery: $batteryPercentage% ${batteryCharging! ? "(Charging)" : ""}"
                          : "Battery: N/A",
                      style: TextStyle(
                          foreground: Paint()..color = Colors.white,
                          fontSize: 24,
                    ...
                    Badge(
                      ...
                      child: Icon(returnBatteryStatusIcon(),
                          color: darkestBlue, size: 45),
                    ),
                    Text(
                      (signalConnection != null)
                          ? ((signalConnection!)
                              ? ""
                              : "Last update: ${DateFormat('dd.MM.yyyy, HH:mm').format(statusUpdateDate!)}")
                          : "",
                      style: TextStyle(
                          foreground: Paint()..color = Colors.white,
                          fontSize: 16,
                  ...
                  child: Column(children: [
                    ...
                    Badge(
                      ...
                      child: Icon(
                          ...
                          color: (signalConnection != null)
                              ? ((signalConnection!) ? darkestBlue : Colors.red)
                              : Colors.orange,
                          size: 45),
                    )
                  ]),
